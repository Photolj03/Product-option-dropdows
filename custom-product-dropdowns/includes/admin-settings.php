...
// Add custom dropdown fields to product edit page
add_action('woocommerce_product_options_general_product_data', function() {
    woocommerce_wp_textarea_input([
        'id' => '_cpd_dropdown_options',
        'label' => 'Custom Dropdown Options',
        'desc_tip' => true,
        'description' => 'Enter each option on a new line, format: Option Name|5'
    ]);
});

// Save custom dropdown fields
add_action('woocommerce_process_product_meta', function($post_id) {
    if (isset($_POST['_cpd_dropdown_options'])) {
        update_post_meta($post_id, '_cpd_dropdown_options', sanitize_textarea_field($_POST['_cpd_dropdown_options']));
    }
});

// Add a settings section for category dropdowns
add_action('admin_init', function() {
    add_settings_section(
        'cpd_category_dropdowns_section',
        'Category Dropdowns',
        null,
        'cpd_settings'
    );
    ...
    foreach ($terms as $term) {
        add_settings_field(
            'cpd_dropdown_options_cat_' . $term->term_id,
            'Dropdown for ' . $term->name,
            function() use ($term) {
                $value = get_option('cpd_dropdown_options_cat_' . $term->term_id, '');
                echo '<textarea name="cpd_dropdown_options_cat_' . $term->term_id . '" rows="4" cols="50" placeholder="Option|Price per line">' . esc_textarea($value) . '</textarea>';
            },
            'cpd_settings',
            'cpd_category_dropdowns_section'
        );
        register_setting('cpd_settings_group', 'cpd_dropdown_options_cat_' . $term->term_id);
    }
});
